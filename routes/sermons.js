...
router.get('/', async (req, res) => {
  try {
    const sermons = await Sermon.find().sort({ title: 1, createdAt: -1 });

    // absolute URL అయితే అలాగే; relative అయితే మాత్రమే base add చెయ్యాలి
    const base = `${req.protocol}://${req.get('host')}/`;
    const normalize = (u) => {
      if (!u) return '';
      return /^https?:\/\//i.test(u) ? u : new URL(u.replace(/^\/+/, ''), base).toString();
    };

    const out = sermons.map((s) => ({ ...s._doc, pdf_url: normalize(s.pdf_url) }));
    const authors = await Sermon.distinct('author');
    const categories = await Sermon.distinct('category');
...
/* ============== POST /api/sermons (field name: pdf) ============== */
router.post('/', upload.single('pdf'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ message: "PDF file is required (field 'pdf')." });
    ...
    // PDFs కోసం 'auto' — Cloudinary సరైన headers తో serve చేస్తుంది
    ...
    const doc = await Sermon.create({
      title: String(title).trim(),
      author: String(author).trim(),
      category: String(category).trim(),
      description: String(description).trim(),
      pdf_url: uploadRes.secure_url, // direct HTTPS URL
    });
